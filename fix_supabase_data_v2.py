...
def sync_data():
    ...
    try:
        ...
        hod = db.query(User).filter(User.username == "241080017").first()
        if not hod:
        ...
        if not fac_teacher:
            db.add(Teacher(id=fac.id, full_name="Prof. V. B. Nikam", department_id="IT", branch="IT"))
        ...
        if not subject:
            subject = Subject(id=sub_id, name="Cloud Computing (Sync)", branch="IT", year="Second Year", department_id="IT")
            db.add(subject)
        else:
            subject.branch = "IT"
        ...
        for i in range(5):
            ...
            if not existing:
                ...
                if not stu:
                    ...
                    db.add(User(id=s_uid, username=reg, full_name=f"Cloud Student {i}", role="student"))
                    db.flush()
                    stu = Student(id=s_uid, registration_number=reg, full_name=f"Cloud Student {i}", branch="IT", year="Second Year", department_id="IT")
                    db.add(stu)
                    db.flush()
        ...
        db.commit()
        print("SUCCESS: Supabase Cloud Data Synced.")
